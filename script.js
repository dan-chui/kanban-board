...
function getSavedColumns() {
  if (localStorage.getItem("backlogItems")) {
    ...
    backlogListArray = ["Relax", "Watch movies"];
    completeListArray = ["Being cool", "Getting stuff done"];
    progressListArray = ["Work on projects", "Take photos"];
...
function updateSavedColumns() {
  ...
  const arrayNames = ["backlog", "complete", "progress", "onHold"];
...
function updateDOM() {
  ...
  completeListArray.forEach((completeItem, index) => {
    createItemEl(completeListEl, 1, completeItem, index);
  });
